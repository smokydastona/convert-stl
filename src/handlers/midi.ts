...
function loadFluidSynth(): Promise<{ JSSynth: any; sfontBin: ArrayBuffer }> {
  if (!midiInitPromise) {
    midiInitPromise = (async () => {
      ...
      let fluidSrc = await fetch(new URL(`${import.meta.env.BASE_URL}wasm/libfluidsynth-2.4.6.js`, globalThis.location.href)).then(r => r.text());
      ...
      await loadScript(new URL(`${import.meta.env.BASE_URL}wasm/js-synthesizer.js`, globalThis.location.href).toString());
      ...
      const sfontBin = await fetch(new URL(`${import.meta.env.BASE_URL}wasm/TimGM6mb.sf2`, globalThis.location.href)).then(r => r.arrayBuffer());
